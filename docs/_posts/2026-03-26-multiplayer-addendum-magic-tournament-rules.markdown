...
**2.2C** For Multiplayer Tournaments where the number of Game Wins required to win a Match is greater than one (Best-of-Three and so on), the seating order remains the same across all Games, but the starting Player is decided using a random method for each Game after the first, as long as the previous Game didn't end in a Draw. **This doesn't apply in Best-of-One Matches**.
...
In Best-of-One Matches, both the seating and the play order don't change in the event of multiple Games being played in the same Match (perhaps due to Drawn Games).
